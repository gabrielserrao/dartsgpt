#!/usr/bin/env python
"""Demonstrate DARTSGPT capabilities with simple rule-based processing."""
...
def demo_generation(prompt: str, output_dir: Path):
...
def main():
    ...
    demo_generation(
        ...
        output_base / "co2_storage"
    )
    
    # Demo 2: Waterflood
    demo_generation(
        "Build a waterflood model with 50x50x5 grid and 200 mD permeability",
        output_base / "waterflood"
    )
    
    # Demo 3: Geothermal
    demo_generation(
        "Model a geothermal reservoir at 150°C with injection and production wells",
        output_base / "geothermal"
    )
