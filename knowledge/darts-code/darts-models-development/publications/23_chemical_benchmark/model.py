...
from darts.physics.base.operators_base import WellControlOperators
...
class Model(DartsModel):
    ...
    def set_initial_conditions(self):
        ...
        for i, comp in enumerate(self.physics.components[:-1]):
            if len(self.map) > 0:
                zc = np.ones(nb) * self.ini_comp[i]
                zc[self.map == 0] = self.ini_void[i]
                input_distribution[comp] = zc
            else:
                input_distribution[comp] = np.ones(nb) * self.ini_comp[i]
        ...
        for i, w in enumerate(self.reservoir.wells):
            if "INJ_GAS" in w.name:
                self.physics.set_well_controls(wctrl=w.control, is_inj=True, control_type=well_control_iface.MOLAR_RATE,
                                               target=self.inj_gas_rate, phase_name='gas', inj_composition=self.inj_stream_gas)
            elif "INJ_WAT" in w.name:
                self.physics.set_well_controls(wtrl=w.control, is_inj=True, control_type=well_control_iface.MOLAR_RATE,
                                               target=self.inj_wat_rate, phase_name='wat', inj_composition=self.inj_stream_wat)
            else:
...
class CustomPhysics(Compositional):
    ...
    def set_operators(self):  # default definition of operators
        ...
        self.well_ctrl_operators = WellControlOperators(self.property_containers[0])
