...
enum mixture { M7, M3, Y8, MY10, C1C16, OILB, MALJAMAR_RES, MALJAMAR_SEP, SOURGAS, BOBSLAUGHTERBLOCK, NORTHWARDESTES, LB, BRANTFERGER};
...
int main(int argc, char **argv)
{
    ...
    // Initialization of output class. It is used for create output files that will be treated later
 	int error_output = 0;
    ...
    else if (std::strcmp(argv[1], "M3") == 0)
    {
 	...
 	else if (std::strcmp(argv[1], "Y8") == 0)
 	{
        ...
        pres = arange<double>(1., 250., 5.);
        temp = arange<double>(150., 450., 10.);
    ...
    else if (std::strcmp(argv[1], "LB") == 0)
    {
        // **For PH test
        //// Hydrocarbon-water mixture (Connoly, 2021), data from (Luo and Barrufet, 2005)
        comp = {"H2O", "PC1", "PC2", "PC3", "PC4"};
        comp_data = CompData(comp);
        ...
        pres = arange<double>(1., 140., 5.);
        temp = arange<double>(300., 600., 5.);
        z_init = {0.5, 0.15, 0.10, 0.10, 0.15};
        diagram_type = PT;
    }
    else if (std::strcmp(argv[1], "BRANTFERGER") == 0)
    {
        // **For PH test
        //// Hydrocarbon-water mixture (Connoly, 2021), data from (Brantferger, 1991)
        comp = {"H2O", "C8", "C13", "C24", "C61"};
        comp_data = CompData(comp);
        comp_data.Pc = {221.20, 34.82, 23.37, 12.07, 7.79};
	    comp_data.Tc = {647.37, 575.78, 698.0, 821.3, 1010.056};
    	comp_data.ac = {0.344, 0.400, 0.840, 1.070, 1.330};
        comp_data.kij = std::vector<double>(5*5, 0.);
        comp_data.set_binary_coefficients(0, {0., 0.5, 0.5, 0.5, 0.5});
        comp_data.cpi = {{32.2, 0.001924, 1.055e-5, -3.596e-9},
                         {-12.3, 0.665, -2.52e-4, 0.},
                         {-5.08, 0.997, -4.14e-4, 0.},
                         {-5.69, 1.84, -7.64e-4, 0.},
                         {0.123, 4.75, -1.95e-3, 0.}};

        pres = arange<double>(1., 140., 5.);
        temp = arange<double>(300., 600., 5.);
        z_init = {0.8, 0.08908, 0.05068, 0.04064, 0.0142};
        diagram_type = PT;
    }
    else
    {
    ...
    // char* ch = argv[1];
    // std::string s(ch);
 	std::string data_out_file_name = "phase_diagram_" + std::string{argv[1]};
    data_out_file_name +=  + ".dat";
    std::ofstream data_out_temp_press_phase(data_out_file_name);

    int nc = comp.size();
 	FlashParams flash_params = FlashParams(comp_data);

    CubicEoS cubic(comp_data, CubicEoS::PR);
 	flash_params.add_eos("PR", &cubic);

 	std::vector<std::string> eos_used{"PR"};
    ...
    flash_params.eos_params["PR"].initial_guesses = {InitialGuess::Yi::Wilson, InitialGuess::Yi::Wilson13,
                                                     0
                                                     };
    // flash_params.verbose = true;

    Flash stab_flash(flash_params);
    std::vector<double> state(nc + 2);
    ...
    if (diagram_type == PT)
    {
        ...
        for (auto p : pres)
        {
            for (auto t : temp)
            {
                ...
                error_output += stab_flash.evaluate(p, t, z);
                FlashResults flash_results = stab_flash.get_flash_results();
    ...
    print("Calculation finished. Error", error_output);
 	data_out_temp_press_phase.close();
